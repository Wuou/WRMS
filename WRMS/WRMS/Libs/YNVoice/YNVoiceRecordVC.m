...
//  LeftSlide
//
//  Created by YangJingchao on 15/10/20.
...
#define PLAYBUTTON_Y (TIME_Y+2*TIME_HEIGHT + SCREEN_HEIGHT/3)
...
@interface YNVoiceRecordVC ()

@end

@implementation YNVoiceRecordVC{
...
- (void)initializeUI {
    ...
    dismissBtn = [[UIButton alloc] initWithFrame:CGRectMake(0, 20, 37, 37)];
    [dismissBtn setImage:[UIImage imageNamed:@"closeVedio.png"] forState:UIControlStateNormal];
    ...
    recordAgainLabel.textColor = [UIColor whiteColor];
...
//    [recordTitleLabel setText:@"录制语音"];
    ...
    timeLabel = [[UILabel alloc] initWithFrame:CGRectMake(SCREEN_WIDTH/2 -TIME_WIDTH/2, TIME_Y, TIME_WIDTH, TIME_HEIGHT)];
...
- (void)transformCAFToMP3 {
    ...
    @finally {
        ...
        NSLog(@"MP3录音文件已生成成功: %@",audioFileSavePath);
       
//        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"mp3转化成功！" message: [NSString stringWithFormat:@"%@",audioFileSavePath] delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
//        [alert show];

        NSDictionary *dictionary = [NSDictionary dictionaryWithObjectsAndKeys:[audioFileSavePath absoluteString],@"VoiceUrl",nil];

        //顺便发起一个通知，刷新隐患上传页面的声音图标
        [[NSNotificationCenter defaultCenter]
         postNotificationName:@"RefreshVoiceIcon" object:nil userInfo:dictionary];
        [self dismissViewControllerAnimated:YES completion:nil];
    }
}
...
//    hud.labelText = @"录音完成";
//    hud.margin = 10.f;
//    CGFloat a = [UIScreen mainScreen].bounds.size.height/3;
//    hud.yOffset = -a;
